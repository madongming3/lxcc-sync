 ...
 * 新建用户和修改密码
 */
function create_new_sync_user($username, $mod, $default_password = USER_DEFAULT_PASSWORD, $s_del,$s_mail,$s_grp) {
    $sql = "insert into " . SYNC_USERS_TABLE ." set " . DB_USERS_USERNAME ." = '".
        $username."', " . DB_USERS_PASSWORD ." = '" .
        md5($default_password) . "', " . DB_USERS_CREATETIME .
        " = unix_timestamp(), " . DB_USERS_MOD . " = " . $mod . "," . DB_USERS_DEL . "=" . $s_del.",".DB_USERS_MAIL
        ."='".$s_mail."',".DB_USERS_GROUP."=".$s_grp;
    return insert_update_db_by_sql($sql);
}
/**
 * 修改用户信息
 */
function user_update($id, $username, $password, $user_mod, $s_del = NULL,$s_mail,$s_grp) {
    $ret         = array();
    ...
    if ( $s_mail != $user_info[DB_USERS_MAIL] )
    	$sql .= sprintf("%s = '%s', ", DB_USERS_MAIL, $s_mail);
    if ( $s_grp != $user_info[DB_USERS_GROUP] )
    	$sql .= sprintf("%s = %d, ", DB_USERS_GROUP, $s_grp);
 ...
 * 判断用户ID获取所属组
 */
function get_user_group_byid($id){
	$sql=sprintf("select %s from %s where %s=%d",DB_PGROUP_NAME,SYNC_PROJECT_GROUP_TABLE,DB_PGROUP_ID,$id);
	$av=get_row_from_db_by_sql($sql);
	if ($av) return $av[DB_PGROUP_NAME];
	else return false;
