...
function list_create($k, $v) {
    ...
    $sql = sprintf("insert into %s (l_hash, l_data) values('%s', '%s')", SYNC_LIST_TABLE,
            $k, mysqli_real_escape_string(_list_fmt($v)));
    $result = mysqli_query($conn, $sql);
    if(!$result || mysqli_affected_rows($conn)) return false;
    return true;
}
/**
 * 修改一条服务器配置
...
function list_update($k, $v) {
    ...
    if(!mysqli_affected_rows($conn)) return false;
    return true;
}
/**
 * 获得一条服务器配置
...
function list_get($k) {
    ...
    $sql = sprintf("select * from %s where l_hash = '%s'", SYNC_LIST_TABLE, mysqli_real_escape_string($k));
    $result = mysqli_query($conn, $sql);
