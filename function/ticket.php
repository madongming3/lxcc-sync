...
function ticket_create_new($t_id) {
    ...
    if($conn){
        ...
        if($result) return mysqli_insert_id($conn);
...
function ticket_db_m($t_id, $file_list = array(), $op_list = array(),
    ...
    $sql = sprintf("update %s set %s = '%s', %s = '%s', %s = '%s', %s = %d , %s = %d  where %s = %d",
                        ...
                        $field, $time, DB_SECTION_STATUS, $status, DB_SECTION_TRAC_ID, $t_id);
 ...
 * 测试状态维护
 */
function ticket_db_teststu($tid,$time,$status){
 ...
 * 记录同步操作日志
 */
function ticket_sync_action($t_id, $hid = null, $action = OP_TYPE_SYNC_TIME,$status=0) {
    ...
    return ticket_db_m($t_id, $fl, $ol, $action, $time,$status);
}
...
function ticket_subtest_action($tid, $hid = null, $action = OP_TYPE_SUBTEST_TIME,$status=1){
	$info = ticket_info($tid);
	$ol = $info[DB_SECTION_OP_LIST];
	$fl    = $info[DB_SECTION_FILE_LIST];
	$array = ticket_op_struct($action, $hid);
	array_unshift($ol, $array);
	#sync_log('Come true '.serialize($ol));
	$time = $array[OP_LIST_KEY_OPTIME];
...
function ticket_info($id, $c = true) {
    ...
    if($ti){
        ...
        $it[DB_SECTION_JENKINS]=ticket_unfmt_l($ti[DB_SECTION_JENKINS]);
    }
    if($ti) $cache[$id] = $ti;
 ...
 * 默认是所有用户
 */
function ticket_get_mine($u = NULL, $page = 1, $rows = 1) {
...
function ticket_test_status($i){
	return 	$i[DB_SECTION_STATUS];
}

function ticket_pass_rate($i){
	return 	$i[DB_SECTION_PASSRATE];
}

function ticket_robot_getpath($i,$id){
	$path=$i[DB_SECTION_ROBOTPATH];
	$sh='robot_run.sh';
	$sh2='/u1/htdocs/www/robot_html/';
	if (strstr($path,$sh)){
		$path=str_ireplace($sh, '', $path);
		if (strstr($path,$sh2)){
			$path=str_ireplace($sh2, '', $path);
		}
		$path=$path.$id;
		return $path;
	}
	return "di zhi bu he fa!";
}

function ticket_last_op_time($i) {
    if(sync_array( $oa = unserialize($i[DB_SECTION_OP_LIST]))){
        $item = $oa[0];
        sync_log(unserialize($item));
...
function ticket_count_robotrate_fromdb($id){
	$sql = sprintf("select %s from %s where id=(select max(id) from %s where %s='%s')",ROBOT_RUN_PASSRATE,ROBOT_TABLE_NAME,ROBOT_TABLE_NAME,ROBOT_RUN_SYNC,$id);
	$conn = get_mysql2_connection();
	if ($conn){
		$result=mysqli_query($conn, $sql);
		if ($result){
...
function ticket_insert_passrate_tosync($rate,$id){
	$sql = sprintf("Update %s set %s=%d where %s=%d",SYNC_TICKETS_TABLE,DB_SECTION_PASSRATE,$rate,DB_SECTION_TRAC_ID,$id);
	$conn = get_mysql_connection();
	if ($conn){
		$result=mysqli_query($conn, $sql);
		if ($result){
...
function ticket_up_robinfo($runro,$id){
		$sql = sprintf("Update %s set %s='%s' where %s=%d", SYNC_TICKETS_TABLE,DB_SECTION_ROBOTPATH,$runro,DB_SECTION_TRAC_ID,$id);  
		$conn = get_mysql_connection();
		if ($conn){
			$result=mysqli_query($conn, $sql);
			if ($result){
...
function ticket_alany_group($fl){
	$av=array();
	foreach ($fl as $v){
		array_push($av,$v[WT_BR]);
	}
	return $av;
}

function get_group_by_prj($prj){
	$sql=sprintf("select %s from %s where %s='%s'",DB_PROJECTS_PGID,SYNC_PROJECTS_TABLE,DB_PROJECTS_NAME,$prj);
	$result=get_row_from_db_by_sql($sql);
	if ($result) return $result[0];
	else return false;
}

function send_mail_by_ssh($tid,$gid){
	$ar=array('ip'=>'127.0.0.1','port'=>'22','user'=>'hui10','passwd'=>'R68TqLU6UfJAJSSLgRRG');
	$conn=ssh_connect_to_server($ar['ip'],$av['port'],$av['user'],$av['passwd']);
	$c = sprintf("", $tid, $gid);
	if($conn){
		$result = ssh2_exec($conn, $c);
		stream_set_blocking($result,1);
		
	}

}

function fmt_jenins($arr=NULL){
    $jav=array();
    $id=TICKET_ID;
    if(!$arr){
        $info=get_jenkins_log($id);
        if(!$info) return false;
        if(count($info>3)){
            $av=array_slice($info,0,3);
            return $av;
        }
        else {
            $av=$info;
...
function save_jenkins_log($info,$status=FALSE){
    $id=TICKET_ID;
    $av=array();
    $arr=array();
    $arr['pro_name']=$info;
    $arr['cr_time']=date('Y-m-d h:m:s',time());
    $arr['cr_user']=cookie_username();
    $status?$arr['status']='成功':$arr['status']='失败';
    $arr['img']='jenkins.png';
    $av[0]=$arr;
    $res=_create_jenkins_log($av,$id);
    if($res){
        return $av;
    }
    return false;
...
function _create_jenkins_log($arr,$id){
    $av=get_jenkins_log($id);
    if($av){
        $av=array_merge($arr,$av);
        sync_log(serialize($av));
        $sql=sprintf("UPDATE sync_tickets SET s_jenkins='%s' WHERE s_trac_id=%d;",serialize($av),$id);
        $res=insert_update_db_by_sql($sql);
        return $res?true:false;
    }
    else {
        $sql=sprintf("UPDATE sync_tickets SET s_jenkins='%s' WHERE s_trac_id=%d;",serialize($arr),$id);
        sync_log($sql);
        $res=insert_update_db_by_sql($sql);
        return $res?true:false;
    }
...
function get_jenkins_log($id){
    $sql=sprintf("SELECT * from sync_tickets WHERE s_trac_id=%d",$id);
    $info=get_row_from_db_by_sql($sql);
    if($info){
        $av=unserialize($info[DB_SECTION_JENKINS]);
        return $av?$av:false;
    }
    return false;
}

function add_key_jenkins($av=Array()){
    $arr[JK_INFO]=$av;
    return $arr;
}
