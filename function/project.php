...
function project_get_by_name($p_name) {
    ...
    if( isset($cache[$p_name]) && $cache[$p_name]) 
        ...
        $sql = sprintf("select * from %s where p_id = '%s' ", SYNC_PROJECTS_TABLE,  $p_name);
        $row = get_row_from_db_by_sql($sql);
        if( sync_array($row) ){
            $cache[$p_name] = $row;
            return $row;
...
function project_get_list($page = FIRST_PAGE, $per_page = LIST_PER_PAGE) {
    $sql = sprintf("select * from %s where %s=%d order by %s desc", SYNC_PROJECTS_TABLE, 
        DB_PROJECTS_STATUS, 0, DB_PROJECTS_ID) . get_limit_str($page, $per_page);
...
function project_get_servers($pname) {
    if($ss = project_get_server_ids_by_name($pname)){
    	#sync_log('ooooooooooooooooooooooooooooooooooooooooooooooooooo'.$ss);
