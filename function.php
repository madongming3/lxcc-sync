...
function output($params = array()) {
    ...
    if(smt_appended_json($params) && SWITCH_JSON){
        if($tpl_file){
            $data = html_fetch_tpl($params, $tpl_file);
        }
 ...
 * 用户列表 追加用户所属组
 */
function tidy_users_group(&$ulist) {
	if ( sync_array($ulist) )
		foreach ( $ulist as $key => &$item )
			$ulist[$key] = tidy_user_group($item);
}
/**
 * 查询出用户的权限
 */
 function tidy_user_group(&$item){
 	if (sync_array($item)){
 		$g_id=(int)$item[DB_USERS_GROUP];
 		if($g_id) $item[DB_USERS_GROUP]=get_user_group_byid($g_id);
 		else $item[DB_USERS_GROUP]='该用户尚未分组';
 		return $item;
 	}
 }
/**
...
function check_usermod($username, $password) {
    ...
    if( sync_array($user_info) &&
            ...
            md5($password) == base64_decode($user_info[DB_USERS_PASSWORD] ))
        return true;
    return false;
}
/**
...
function svn_version_file_list($version, $pn, $sort = null) {
    ...
    $nr = 1;
    sync_svn_auth($si);
    #sync_log('url::'.$si[SVN_INFO_KEY].'  '.$version.'  '.$nr);
 ...
 * 取某个用户的上线单列表
 */
function ticket_list_u($username = null, $page = 1, $rows = 1) {
...
function ticket_detail($id, $nfl = true) {
    ...
    if($ti){
        ...
        $r[JK_INFO]=fmt_jenins(unserialize($ti[DB_SECTION_JENKINS]));
        ...
        $r[WT_STATUS]=ticket_test_status($ti);
        $r[WT_PASS]=ticket_pass_rate($ti);
        $r[WT_PATH]=ticket_robot_getpath($ti,$id);
    }
    return $r;
}
/**
 ...
 * 选择jenkins的首屏
 */
function get_jenkins_info(){
    $r=array();
    $sql='SELECT p_jenkins_key FROM sync_jenkins_pro WHERE p_status=1;';
    $jl=get_row_from_db_by_sql($sql,true);
    if(sync_array($jl)){
        $res=[];
        foreach ($jl as $v){
            $k=$v['p_jenkins_key'];
 ...
 */
    $_ACTION_TYPE_TO_CHAR = array(
    		...
    		OP_TYPE_SUBTEST_TIME    => '提交测试',
    		OP_TYPE_PASS_TIME       => '@测试通过!',
    		OP_TYPE_FAILED_TIME     => '@测试不通过!',
    		OP_TYPE_ROBOT_TIME      => '@robot测试执行'
            );
...
function sync_format_server_info($s_id) {
    ...
    if($s_info){
        ...
        if(server_svn($s_info[DB_SERVERS_TYPE])){
            ...
            #sync_log('fanhuizhi1:'.$ret[SVN_INFO_KEY].'    fan hui zhi 2'.$ret[SVN_INFO_KEY_TRUNK].'   fan hui zhi 3:'.$ret[SVN_INFO_KEY_PREFIX]);
    ...
    sync_log('_+_+_+_+_+_++_+_+__'.serialize($ret));
    return $ret;
}

/**
 * 根据robot的id获得服务器信息的数据结构
 * 如果给定的数据结构是数组，那么直接进行格式化
 */
function sync_format_robot_info($s_id) {
	$ret = array();
	$s_info = sync_array($s_id) ? $s_id : robot_get($s_id);
	if($s_info){
		$ret[ROBOT_INFO_KEY_ID] = $s_info[DB_ROBOT_ID];
		$ret[ROBOT_INFO_KEY_IP]       = ($s_info[DB_ROBOT_IP]);
		$ret[ROBOT_INFO_KEY_PORT]     = ($s_info[DB_ROBOT_PORT]);
		$ret[ROBOT_INFO_KEY_WWWROOT]  = sync_trim_path($s_info[DB_ROBOT_PATH]);
		#$ret[SVN_INFO_KEY_FILESE]           = true;
		$ret[ROBOT_KEY_CLOSE]            = $s_info[DB_ROBOT_DEL];
		$ret[ROBOT_INFO_KEY_DESC]             = sync_trim_path($s_info[DB_ROBOT_DESC]);
		$ret[ROBOT_INFO_KEY_USERNAME] = sync_trim_path($s_info[DB_ROBOT_USER]);
		$ret[ROBOT_INFO_KEY_PASSWORD] = sync_trim_path($s_info[DB_ROBOT_PASSWD]);
	}
 ...
 * robot是否是在使用状态
 */
function robot_use($sid) {
	$ret    = false;
	#$robot = robot_get($sid);
	#if ( is_array($robot) && $robot[DB_ROBOT_ID] > 0 ) {
		#$s_list = project_list_by_server($sid, $s_type);
		#if ( sync_array($s_list) )
			#$ret = true;
	#}
...
function get_post_dt() {
    ...
    case 'console/user':
        ...
        $dt['mail']=(string)$_REQUEST[USER_INPUT_MAIL];//获取邮箱地址
        $dt['sel_grp']=(int)$_REQUEST[USER_INPUT_GROUP];//获取分组ID
        break;
    case 'console/guide':
        if ( CURR_ACT == 'step1' ) 
 ...
 * 获取创建server提交的post数据
 */
 function get_robot_dt(&$dt){
 	$dt['id'] = CONSOLE_ID;
 	$dt['nr'] = (string)$_REQUEST[ROBOT_INPUT_NAME];//名称
 	$dt['ip'] = (string)$_REQUEST[ROBOT_INPUT_IP];//IP
 	$dt['pt'] = (int)$_REQUEST[ROBOT_INPUT_PORT];//端口
 	$dt['dz'] = (string)$_REQUEST[ROBOT_INPUT_PREFIX];//路径
 	$dt['un'] = (string)$_REQUEST[ROBOT_INPUT_USER_NAME]; //用户名
 	$dt['pw'] = (string)$_REQUEST[ROBOT_INPUT_PASSWORD];//密码
 	$dt['st'] = (int)$_REQUEST[ROBOT_INPUT_IS_DEL];
...
function _val_robot_dt(&$dt){
	$r = array();
	if ( !_check_str_length($dt['nr']) ) {
		err_set(WARING_CODE);
		$r[ROBOT_INPUT_NAME]   = SERVER_MESSAGE_DESC_ERR;
	} else if ( robot_desc_exist($dt['nr'], $dt['id']) ) {
		err_set(WARING_CODE);
		$r[ROBOT_INPUT_NAME]   = ROBOT_MESSAGE_DESC;
	}

	if(!$dt['ip']) $r[ROBOT_INPUT_IP_URL]      = '主机地址不能为空';
	if(!$dt['un']) $r[ROBOT_INPUT_USER_NAME]   = '用户名不能为空';
	if(!$dt['pw']) $r[ROBOT_INPUT_PASSWORD]    = '密码不能为空';
	if(!$dt['dz']) $r[ROBOT_INPUT_PREFIX]    = '路径不能为空';	
	if(!$dt['pt']) $r[ROBOT_INPUT_PORT]        = '服务器端口不能为空';
	if(!$dt['ip'] || !$dt['un'] || !$dt['pw'] ||!$dt['pt'] ){
		err_set(1);return $r;
	}	
	$err = ssh2_robot_info($dt['ip'], $dt['un'], $dt['pw'], $dt['dz'],
			 $dt['pt']);	
	if ( $err != 0 ) {
		err_set($err);
		switch($err) {
			case 1:
				$r[ROBOT_INPUT_PORT]    = ssh2_errmsg($err);
				break;
			case 2:
				$r[ROBOT_INPUT_USER_NAME] = ssh2_errmsg($err);
				break;
			case 3:
			case 7:
				$r[ROBOT_INPUT_PREFIX] = ssh2_errmsg($err);
				break;
			} 
		}else{
			$ff = ssh2_server_struct($dt['ip'], $dt['pt'], $dt['un'],
...
function robot_create($dt, &$err) {
		if ( $dt['id'] > 0 ){
			$rt = robot_update($dt['id'], $dt['nr'], $dt['ip'], $dt['pt'], $dt['dz'], $dt['un'],
					$dt['pw'], $dt['st']);
			sync_log('$dt[pt]'.$dt['un']);
		}
		else
			$rt = robot_create_new($dt['nr'], $dt['ip'], $dt['pt'], $dt['dz'], $dt['un'],
					$dt['pw'], $dt['st']);
		parse_rt($rt);
		return get_last_id();
	}
...
function user_create($dt) {
    if ( $dt['id'] > 0 )
        $rt = user_update($dt['id'], $dt['un'], $dt['pd'], $dt['ud'], $dt['cl'],$dt['mail'],$dt['sel_grp']);
    else
        $rt = create_new_sync_user($dt['un'], $dt['ud'], $dt['pd'], $dt['cl'],$dt['mail'],$dt['sel_grp']);
    parse_rt($rt);
    return $rt;
}
/**
...
function console_robot_lists($page = FIRST_PAGE, $row = DEFAULT_PER_PAGE) {
	$lists = $ret = array();
	$order = DB_ROBOT_DEL . " asc," . DB_ROBOT_ID . " desc";
	$lists = robot_lists($page, $row, $order);
	if ( sync_array($lists) )
		foreach ( $lists as $k => $v )
			$ret[] = sync_format_robot_info($v);
		return $ret;
}
/**
/**
...
function robot_path_check(){
	$cache=array();
	$sql='select * from '.ROBOT_SERVER_TABLE;
	$cache=get_row_from_db_by_sql($sql,true);
	return $cache;	
}

function robot_run_check(){
	$av=array();
	$rid=(int)$_REQUEST['robot'];
	if (!$rid) return '没有传robot id,系统错误';
	$pathinfo=get_robot_info_byid($rid);
	if(robot_run_do($pathinfo)){
		smt_append_value($pathinfo,1,'is_suc');
		$av=$pathinfo;
		return $av;
	}
	else {
...
function ticket_count_robotres($id){
	$rate=ticket_count_robotrate_fromdb($id);
	if ($rate){
		ticket_insert_passrate_tosync($rate,$id);
		return true;
	}
	return false;
...
function ticket_get_report_path($id){
	$path=get_robot_info_byid($id);
		if (!$path){
			return "wei zhao dao jie guo!";#后期改进
		}
	
}

function ticket_update_robotinfo($runro,$id){
	static $cache=array();
	if( isset($cache['ip']) && $cache['ip'])
		return 1;
	$cache['ip'] = (string)$runro['r_ip'];
	$cache['path'] = (string)$runro['r_path'];
	$res=ticket_up_robinfo($cache['path'],$id);
	if($res){
		return true;
	}
	return false;
