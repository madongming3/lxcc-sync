...
function web_ticket_subtest(){
	$av=array();
	$r=array();
	$id=TICKET_ID;
	$t_info=ticket_subtest_action($id);
	$r = ticket_detail(TICKET_ID);
	smt_append_ticket($av, $r);
	smt_append_json($av,_web_tpl('ticket_detail_fls'));
	return $av;
}

function web_ticket_runtest(){
	$av=array();
	$r=array();
	$id=TICKET_ID;
	if($_REQUEST['status']){
		ticket_subtest_action($id,null,$action=OP_TYPE_FAILED_TIME,$_REQUEST['status']);
	}else{
		ticket_subtest_action($id,null,$action=OP_TYPE_PASS_TIME,2);
	}
	smt_append_ticket($av, $r);
	smt_append_json($av,_web_tpl('ticket_detail_fls'));
	return $av;
...
function web_ticket_run_robot(){
	$av=array();
	$tid=TICKET_ID;
	$runro=robot_run_check();
	if($runro) ticket_robottest_action($tid);
	else return "yun xing shi bai!!";
	ticket_count_robotres($tid);
	ticket_update_robotinfo($runro,$tid);
	ticket_send_mail($tid);
	$r=$runro;
	smt_append_ticket($av, $r);
	smt_append_json($av);
	return $av;
...
function web_ticket_selrobot(){
	$av=array();
	$path=robot_path_check();
	smt_append_ticket($av, $path);
	smt_append_json($av);
	return $av;	
}

function web_ticket_robot_clo(){
    $av = array();
    $id = TICKET_ID;
	smt_append_ticket($av, $r);
	smt_append_json($av,_web_tpl('ticket_detail'));
	return $av;
}


function web_ticket_sigmd5(){
	$av=array();
	$au=$_REQUEST['sig'];
	$res=ticket_auth_md5($au);
	if ($res){
		$av['code']=20;
		$av['auth']=$res;
	...
	$av['code']=1002;
	$av['auth']='Chuan shu bu he fa!!';
	return json_encode($av);
}

function web_ticket_jenkins(){
    $id=TICKET_ID;
    $jp=get_jenkins_info();
    if(!$jp) { error_json('empty_pjgrp', $av); return $av; }
    smt_append_list($av, $jp);
    smt_append_json($av);
    return $av;
}
	
function web_ticket_build(){
    $av=array();
    $id=TICKET_ID;
    $web_project=$_POST['jen_name'];
    $jenkins=new jenkins();
    $res=$jenkins->info_lottery($web_project);
    if($res){
        $jres=save_jenkins_log($web_project,true);
        if($jres){
            $list=fmt_jenins();
            smt_append_ticket($av, add_key_jenkins($list));
            smt_append_json($av,_web_tpl('ticket_jenkins_fls'));
            return $av;
        }
    }
    return false;
