...
(function($) {
	...
	$('input[name="mail"]').inspect({
		...
		'error': function() {
			var $wrap = this;
			$wrap.next('.error').html('邮件地址不能为空！').addClass('text-error').removeClass('corrects').show();
		}
	});

	//检查group
	$('input[name="user_group"]').inspect({
		'min': 1,
		'success': function() {
			var $wrap = this;
			$wrap.next('.error').html('').addClass('corrects').removeClass('text-error').show();
		...
		'error': function() {
			var $wrap = this;
			$wrap.next('.error').html('请输入正整数值！').addClass('text-error').removeClass('corrects').show();
		}
	});
