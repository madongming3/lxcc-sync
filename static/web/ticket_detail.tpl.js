...
;(function($) {
	...
	/*点击提交测试*/
	$('a[action-type="act-submit-test"]').on('click', function(e) {
		
		var $this = $(this);
		var data = { 'id' : $config.id };
		var url='index.php?mod=web.ticket&act=detail';
...
//返回值成功，但是未触发detail页面
		$subtest.onok(function() {
			$('#submit_test').ajaxSubmit({
				'success': function($data) {
					$data=$.parseData($data);
					var type=$.type($data);
					if (type=='object'){
	...
	/*点击测试通过*/
	$('a[action-type="act-run-test"]').on('click', function(e) {
	...
	/*点击测试不通过*/
	$('a[action-type="act-run-testfail"]').on('click', function(e) {
	...
	/*选择robot路径*/
	$('a[action-type="act-sel-robot"]').on('click', function(e){
...
/*执行robot*/
	$('body').on('click', 'a[action-type="act-run-robot"]', function(e) {
		var $request = null;
		if ($request) return false;
		$("h2").show();
		...
		$('#run-robot').attr('action','/index.php?mod=web.ticket&act=run_robot&id=' + $config.id).ajaxSubmit(option);
	});
	...
	$('body').on('click', 'a[action-type="act-save-jenkins"]', function(e) {
		var $request = null;
		if ($request) return false;
		...
		$('#select-jenkins').ajaxSubmit(option);
	});
	
	/*刷新页面*/
	$('body').on('click', 'a[action-type="act-robot-true"]', function(e) {
		var $request = null;
		if ($request) return false;
