...
function ssh_connect_to_server($ip, $port, $user, $pwd, $pkfile = null, $prikfile = null) {
    ...
    if($conn){
    	...
    	sync_log('[][Lian jie cheng gong!!!');
    }else{
    	sync_log('[][Lian jie shi bai!!!!');
    }
...
function ssh_scp_dir_file($conn, $lfile, $rfile, $isdir = null, $mode = 0755) {
    ...
    if($isdir){ 
    	...
    	$result = ssh_scp_mkdir($conn, $rfile, $mode);}
...
function ssh_scp_send_file($conn, $lfile, $rfile, $mode) {
    ...
    #sync_log("oooooooooooooo zhi xing de shi zhe ge tiaojian!!aaaaaa");
    return $result;
}
/**
 * 创建一个目录
 ...
 * robot服务器信息验证
 */
function ssh2_robot_info($host,$u,$p,$dir,$port =22){
	if(!$host) return 6;
	if(!$p) return 4;
	if(!$dir) return 5;
	if(!sockalive($host, $port)) return 1;
	...
	$r = ssh_robot_is_write($conn, $dir);
	if($r === null) return 3;
	elseif($r === false) return 7;
	return 0;
}
/**
 * robot服务器信息验证文件是否存在，可执行
 */
function ssh_robot_is_write($conn,$dir){
	if(!ssh_file_exists($conn, $dir)) return null;
	return true;
}
/**
