 ...
 * 对svn函数做了封装
 */
function sync_svn_log($url, $s_version,
        $e_version, $limit = 0) {
    if(svn_server_alive($url)) return false;
    $ret = null;
    #sync_log('ci yi ci::'.$url.' '.$s_version.'  '.$e_version.'  '.$limit);
    $ret = svn_log('http://223.223.184.85:81/svn/test_center/trunk',$s_version,$e_version,1);##问题就出现在这一行
    #sync_log('ban ben hao::'.serialize($ret));
    return $ret;
